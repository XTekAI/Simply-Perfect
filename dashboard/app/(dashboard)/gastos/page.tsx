...
export default function GastosPage() {
  const { rows, loading } = useRealtimeTable<Gasto>("sp_gastos");
  ...
  async function handleMarcar(id: string) {
    setMarcando(id);
    try {
      await marcarRevisado("gastos", id);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-serif font-bold text-gray-900 dark:text-pink-200">Gastos</h1>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <p className="text-center text-gray-400 dark:text-pink-300/50 py-12">
            No hay facturas de gasto todavía.
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtrados.map((g) => (
                  ...
                  >
                    <td className="px-5 py-3 font-mono text-xs text-gray-400">{g.numero_factura_interno}</td>
                    <td className="px-5 py-3 text-gray-600 dark:text-pink-200/80">{formatDate(g.fecha_factura)}</td>
                    <td className="px-5 py-3 text-gray-800 dark:text-pink-100 font-medium">
                      {g.proveedor || "—"}
                      <p className="text-xs text-gray-400 dark:text-pink-300/50 font-normal truncate max-w-[200px]">
                        {g.concepto}
                      </p>
                    </td>
                    <td className="px-5 py-3 text-gray-600 dark:text-pink-200/80">{g.categoria || "—"}</td>
                    <td className="px-5 py-3 text-right font-semibold text-gray-900 dark:text-pink-100">
                      {formatMoney(Number(g.total), g.moneda)}
                    </td>
                    <td className="px-5 py-3">
                      <ConfianzaBadge confianza={g.confianza} />
                    </td>
                    <td className="px-5 py-3">
                      <EstadoBadge estado={g.estado_revision} />
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {g.estado_revision !== "revisado" && (
                          <button
                            onClick={() => handleMarcar(g.id)}
                            disabled={marcando === g.id}
      ...
      {editando && (
        <EditInvoiceModal tabla="gastos" invoice={editando} onClose={() => setEditando(null)} />
      )}
    </div>
  );
}
